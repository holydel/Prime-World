...
namespace nival
{
	...
	{
		//return static_cast< T* >( InterlockedExchangePointer( &value, new_value ) ); holydel
		return static_cast<T*>(_InlineInterlockedExchangePointer((volatile PVOID*) & value, new_value));
	}

	template< typename T > inline T* interlocked_compare_exchange_pointer(T* volatile& destination, T* exchange, T* comparand)
	{
		//return static_cast< T* >( ::InterlockedCompareExchangePointer( (volatile PVOID *)&destination, exchange, comparand ) ); holydel
		return static_cast<T*>(::_InlineInterlockedCompareExchangePointer((volatile PVOID*)&destination, exchange, comparand));
	}
