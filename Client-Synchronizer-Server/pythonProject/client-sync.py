...
def api():
    ...
    if request.method == 'POST':
        ...
        if method == 'getDataUsers':
            reqJson = json.loads(request.data.decode(encoding='utf-8', errors='ignore'),
                                 strict=False)  # for some reason encoding is not valid here
            data = reqJson["data"]
        ...
        if method == "registerUserInSession":
            ...
            if sessionToken not in activeSessionTokens:
                tloc = threading.local()
                if sessionToken not in activeSessionTokens:
                    activeSessionTokens[sessionToken] = {'token': sessionToken, 'gameName': '', 'players': [
                        {'nickname': data["nickname"], 'heroId': data["heroId"]}], 'timestamp': dt.now()}
                    response = {
                        'error': '',
                        'data': ''
                    }
                ...
                if session['gameName'] == '':  # wait for creator
                    ...
                    for player in activeSessionTokens[sessionToken]['players']:
                        if player['nickname'] == data["nickname"]:  # player already registered in this session
                            ...
                            return jsonify(response)
                    activeSessionTokens[sessionToken]['players'].append(
                        {'nickname': data["nickname"], 'heroId': data["heroId"]})
                    response = {
                        'error': '',
        ...
        if method == "lobbyCreated":
            ...
            if sessionToken in activeSessionTokens:
                ...
                if session['gameName'] == '':  # save gameId
                    activeSessionTokens[sessionToken]['gameName'] = data[
                        "nickname"]  # just save creator's nickname for now
                    response = {
                        'error': '',
                        'data': 0
                    }
